...
public class Utils {
        ...
        while (true) {
            try {
                ...
                if (!id.matches("P[0-9]{3}")) {
                    throw new Exception("Try again without space (Ex: P101)");
                }
                return id;
            } catch (Exception e) {
    ...
    private static boolean isOnlyAlphabet(String fullName) {
        ...
        for (int i = 0; i < fullName.length(); i++) {
            ...
            if (!Character.isLetter(fullName.charAt(i))) return false;
        }
        return true;
    }
    ...
    public static String getName(String msg) {
        while (true) {
            try {
                ...
                if (isOnlyAlphabet(fullName) || (isOnlyAlphabet(fullName) && fullName.contains(" "))) return fullName;
                else {
